...
class BoardGenerator:
    ...
    def build(self, board_size_mm, start_id, family, out_prefix, split=False):
        ...
        for item in layout_data:
            tag_id, cx_px, cy_px, size_px = item['id'], item['cx'], item['cy'], int(item['size'])
            ...
            half_s_m = (size_px / 2.0) * self.pixel_to_meter
        ...
        exporter.export_svg(save_dir.joinpath(f"{out_prefix}_full.svg"), downloader_cache)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--layout", choices=layout_keys, default=layout_keys[-1], help="布局")
    parser.add_argument("--family", default="tag16h5", help="AprilTag族 (默认: tag16h5, 即 tag5)")
    parser.add_argument("--size-mm", type=float, default=600.0, help="画板的实际物理边长(毫米)，默认 600mm")

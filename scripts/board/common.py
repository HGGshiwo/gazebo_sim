...
class AprilTagDownloader:
    ...
    # 在当前脚本运行目录下创建缓存文件夹
    CACHE_DIR = Path(__file__).parent.joinpath(".tag_cache")

    @staticmethod
    def fetch_tag(tag_id, family="tag16h5"):
        # 确保缓存目录存在
        AprilTagDownloader.CACHE_DIR.mkdir(parents=True, exist_ok=True)
        ...
        cache_path = AprilTagDownloader.CACHE_DIR.joinpath(filename)
        ...
        print(f"  -> 正在从网络获取并缓存 [{family}] ID: {tag_id}...")
...
class Exporter:
    ...
    def export_svg(self, output_path, downloader_cache):
        ...
        for item in self.layout_data:
            ...
            buffered = BytesIO()
            img.resize((int(item["size"]), int(item["size"])), Image.NEAREST).save(
                buffered, format="PNG"
            )
            ...
            svg.append(
                f'<image x="{px}" y="{py}" width="{item["size"]}" height="{item["size"]}" href="data:image/png;base64,{b64_str}" />'
            )
    ...
    def export_pdf_split(self, output_path):
        ...
        for r in range(rows):
            for col in range(cols):
                ...
                part = self.canvas_img.crop(
                    (left, upper, left + a4_w_px, upper + a4_h_px)
                )
